...
class SecordStep extends Component {
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        if (nextProps.Sid !== prevState.Sid) {
            return { Sid: nextProps.Sid }
        }
        return null;
    }
    ...
    handleSubmit = e => {
        ...
        this.props.form.validateFieldsAndScroll((err, values) => {          //这里的values采用的是{medID:num}这样的对象形式，标识有多少这个药品
            if (!err) {
                ...
                axios('/StudentController/updatePwdStep2', { Sid, ...values, type: 'Student' }, 'POST')
                    .then(resp => {
                        if (resp.data > 0) {
    ...
    componentDidMount() { this.setState({ Sid: this.props.Sid }) }
